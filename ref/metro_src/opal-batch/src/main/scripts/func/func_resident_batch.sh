...
###############################################################################
#  Script name  : func_resident_batch.sh
#  Description  : Javaで実装された常駐型バッチプログラムを起動する
#  Usage        : func_resident_batch.sh JOB_ID REQUEST_PATH USER_ID [PARAM_MAIN]...
#                     JOB_ID
#                       ジョブID
#                     REQUEST_PATH
#                       リクエストパス
...
#  Author       : Wataru Ito
#  Returns      : 0   正常終了
#               : 1~  異常終了(常駐型バッチプログラム起動時に生じた終了コード)
###############################################################################
...
### 実行オプションの生成
NAB_OPT="-classpath ${RUNTIME_CLASSPATH} ${RESI_OPT} ${INP_NAB_OPT} "
...
### Mainクラス引数を設定
MAIN_ARGS="-diConfig ${RESI_DI_CONF}"
