...
###############################################################################
#  Script name  : func_copy_file.sh
#  Description  : ファイルをコピーまたは移動する
#  Usage        : func_copy_file.sh SEND_DIR_PATH INPUT_FILE_NAME RECEIVE_DIR_PATH MOVE_FILE_NAME OVERWRITE_FLG PROC_PATTERN
#                     SEND_DIR_PATH
#                       コピー・移動元ディレクトリパス（環境ごとのベースディレクトリからのパス）
#                     INPUT_FILE_NAME
#                       コピー・移動元ファイル名
#                     RECEIVE_DIR_PATH
#                       コピー・移動先ディレクトリパス（環境ごとのベースディレクトリからのパス）
#                     MOVE_FILE_NAME
#                       コピー・移動先ファイル名
#                     OVERWRITE_FLG
#                       コピー・移動先ファイルの上書き可否
...
#                 111 異常終了(上書き可否引数不正)
#                 112 異常終了(ファイル処理方法不正)
#                 113 異常終了(コピー・移動元ディレクトリ変数名不正、または
#                              コピー・移動元ディレクトリが存在しない場合)
#                 114 異常終了(コピー・移動先ディレクトリ変数名不正、または
#                              コピー・移動先ディレクトリが存在しない場合)
#                 115 異常終了(コピー・移動元ファイル未存在)
#                 116 異常終了(コピー・移動先に同名ファイル存在)
...
if [ "${OVERWRITE_FLG}" -eq 1 ] && [ "${PROC_PATTERN}" -eq 1 ]; then
    cp -f ${INPUT_FILE_PATH} ${MOVE_FILE_PATH}
elif [ "${OVERWRITE_FLG}" -eq 0 ] && [ "${PROC_PATTERN}" -eq 1 ]; then
    cp ${INPUT_FILE_PATH} ${MOVE_FILE_PATH}
