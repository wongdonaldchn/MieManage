 ...
 * @since 1.0
 */
public class B114A011Action extends FileBatchAction {

    /** ファイル名 */
    private static final String FILE_ID = "A114A001";

    /** フォーマットファイル名(拡張子除く) */
    private static final String FORMAT_ID = "A114A001";

    /** エラーフォーマットファイル名(拡張子除く) */
    private static final String ERROR_FORMAT_ID = "A114A001_ERROR";

    /** バッチ処理ID */
    private static final String BATCH_PROCESS_ID = "B114A011";

    /** バッチエラーファイル名称 */
    private StringBuffer batchErrorFileName = new StringBuffer("A114A001_error_");

    /** 入力データ件数(OP会員情報データレコード) */
    private int inputDataCount;

    /** 出力データ件数(OP会員一時情報)(登録) */
    private int insertOpMemTempInfoCount;

    /** レコード存在確認用 */
    private boolean isExistRecord;

    /** OP会員情報レコード精査エラー存在確認用 */
    ...
    protected void initialize(CommandLine command, ExecutionContext context) {
        ...
        this.insertOpMemTempInfoCount = 0;
    ...
    public Result doData(DataRecord inputData, ExecutionContext ctx) {
        ...
        // inputDataをFormに展開する
        A114A001DataForm dataForm = BeanUtil.createAndCopy(A114A001DataForm.class, inputData);
    ...
    protected void terminate(Result result, ExecutionContext context) {
        ...
        if (result.isSuccess()) {
            if (!this.isExistRecord) {
                writeLog("AB114A0102");
                throw new TransactionAbnormalEnd(101, "AB114A0102");
            }
        }

        // 出力データ件数(OP会員一時情報)(登録)
        writeLog("MB114A0106", this.insertOpMemTempInfoCount);

        // OP会員情報レコードで精査エラーが存在する場合は、エラーを出力する。
        if (this.isExistError) {
            throw new TransactionAbnormalEnd(100, "AB114A0101");
        }
    }
     ...
     *            入力データ
     */
    private void insertOpMemTempInfo(DataRecord inputData) {
        ...
        // データを登録する
        ParameterizedSqlPStatement statement = super.getParameterizedSqlStatement("INSERT_OP_MEM_TEMP_INFO");
        this.insertOpMemTempInfoCount += statement.executeUpdateByMap(inputData);
    }
    ...
    public class FileLayoutValidatorAction implements ValidatableFileDataReader.FileValidatorAction {
        ...
        /** データレコード */
        private static final String DATA_RECORD_MSG = "OP会員情報レコード";
        ...
        public Result doData(DataRecord inputData, ExecutionContext ctx) {
            ...
            // 入力データ件数(OP会員情報データレコード)をカウントアップする。
            B114A011Action.this.inputDataCount++;
        ...
        public void onFileEnd(ExecutionContext ctx) {
            ...
            if (!END_RECORD.equals(this.preRecordKbn)) {
                // 最終レコードがエンドレコードで無い場合
                writeLog("MB114A0103", END_RECORD_MSG);
                throw new TransactionAbnormalEnd(100, "AB114A0101");
            }

            // 入力データ件数(OP会員情報データレコード)
            writeLog("MB114A0105", B114A011Action.this.inputDataCount);
        }
        ...
        private void checkHeader(DataRecord inputData) {
            ...
            // inputDataをFormに展開する
            A114A001HeaderForm headerForm = BeanUtil.createAndCopy(A114A001HeaderForm.class, inputData);
            ...
            if (!this.fileId.equals(inputData.getString("fileId"))) {
                // ファイルIDのその他精査はエラーとする
                writeLog("MB114A0104", HEADER_RECORD_MSG, FILE_ID_MSG);
                throw new TransactionAbnormalEnd(100, "AB114A0101");
            }
        }
        ...
        private void checkData(DataRecord inputData) {
            ...
            if (!HEADER_RECORD.equals(this.preRecordKbn) && !DATA_RECORD.equals(this.preRecordKbn)) {
                ...
                writeLog("MB114A0102", DATA_RECORD_MSG);
                throw new TransactionAbnormalEnd(100, "AB114A0101");
            }
        }
        ...
        private void checkTrailer(DataRecord inputData) {
            ...
            if (!HEADER_RECORD.equals(this.preRecordKbn) && !DATA_RECORD.equals(this.preRecordKbn)) {
                // 前レコードがヘッダー、データで無い場合
                writeLog("MB114A0102", TRAILER_RECORD_MSG);
                throw new TransactionAbnormalEnd(100, "AB114A0101");
            }
            ...
            // レコード件数と一致すること
            if (B114A011Action.this.inputDataCount != Integer.parseInt(dataRecordCnt)) {
                // レコード件数と一致しない場合
                writeLog("MB114A0104", TRAILER_RECORD_MSG, DATA_RECORD_CNT_MSG);
                throw new TransactionAbnormalEnd(100, "AB114A0101");
            }
        }
