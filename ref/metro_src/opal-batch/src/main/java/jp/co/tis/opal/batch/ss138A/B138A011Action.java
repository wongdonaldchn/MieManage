 ...
 * @since 1.0
 */
public class B138A011Action extends FileBatchAction {

    /** ファイル名 */
    private static final String FILE_ID = "A138A001";

    /** フォーマットファイル名(拡張子除く) */
    private static final String FORMAT_ID = "A138A001";

    /** エラーフォーマットファイル名(拡張子除く) */
    private static final String ERROR_FORMAT_ID = "A138A001_ERROR";

    /** バッチ処理ID */
    private static final String BATCH_PROCESS_ID = "B138A011";

    /** バッチエラーファイル名称 */
    private StringBuffer batchErrorFileName = new StringBuffer("A138A001_error_");

    /** 入力データ件数(PiTaPa利用実績情報レコード) */
    private int inputDataCount = 0;

    /** 出力データ件数(PiTaPa利用実績一時情報)(登録) */
    private int insertPitapaUseResTempInfoCount;

    /** レコード存在確認用 */
    private boolean isExistRecord;

    /** PiTaPa利用実績情報レコード精査エラー存在確認用 */
    ...
    protected void initialize(CommandLine command, ExecutionContext context) {
        ...
        // 件数の初期値を設定する
        this.insertPitapaUseResTempInfoCount = 0;
    ...
    public Result doData(DataRecord inputData, ExecutionContext ctx) {
        ...
        // inputDataをFormに展開する
        A138A001DataForm dataForm = BeanUtil.createAndCopy(A138A001DataForm.class, inputData);

        try {
            // バリデーションを実行する
            ValidatorUtil.validate(dataForm);

            // データレコードをPiTaPa利用実績一時情報TBLに登録する
            insertPitapaUseResTempInfo(inputData);
            ...
            for (Message message : e.getMessages()) {
                ...
                keyMap.put("会員管理番号", inputData.getString("memControlNum"));
                keyMap.put("会員管理番号枝番", inputData.getString("memControlBrNum"));
                keyMap.put("PiTaPaご利用年月", inputData.getString("pitapaUseYearMonth"));
                ...
                memberControlNumber.append(inputData.getString("memControlNum"));
                // 会員管理番号枝番
                StringBuffer memrCtrlBrNum = new StringBuffer("memrCtrlBrNum:");
                memrCtrlBrNum.append(inputData.getString("memControlBrNum"));
                // PiTaPaご利用年月
                StringBuffer pitapaUseYearMonth = new StringBuffer("pitapaUseYearMonth:");
                pitapaUseYearMonth.append(inputData.getString("pitapaUseYearMonth"));
                // メッセージ
                ...
                writeRecord(line.toString(), memberControlNumber.toString(), memrCtrlBrNum.toString(),
                        pitapaUseYearMonth.toString(), errorMessage.toString());
            }
        }
        return new Result.Success();
    }
     ...
     *            メッセージ
     */
    private void writeRecord(String line, String memberControlNumber, String memrCtrlBrNum, String pitapaUseYearMonth,
            String message) {
        ...
        errorMap.put("pitapaUseYearMonth", pitapaUseYearMonth);
    ...
    protected void terminate(Result result, ExecutionContext context) {
        ...
        if (result.isSuccess()) {
            if (!this.isExistRecord) {
                writeLog("AB138A0102");
                throw new TransactionAbnormalEnd(101, "AB138A0102");
            }
        }

        // 出力データ件数(PiTaPa利用実績一時情報)(登録)
        writeLog("MB138A0106", this.insertPitapaUseResTempInfoCount);

        // PiTaPa利用実績情報レコードで精査エラーが存在する場合は、エラーを出力する。
        if (this.isExistError) {
            throw new TransactionAbnormalEnd(100, "AB138A0101");
        }
    }
     ...
     *            入力データ
     */
    private void insertPitapaUseResTempInfo(DataRecord inputData) {
        // DBに登録する値を設定する。
        // 処理済フラグ
        ...
        // データを登録する
        ParameterizedSqlPStatement statement = super.getParameterizedSqlStatement("INSERT_PITAPA_USE_RES_TEMP_INFO");
        this.insertPitapaUseResTempInfoCount += statement.executeUpdateByMap(inputData);
    }
    ...
    public class FileLayoutValidatorAction implements ValidatableFileDataReader.FileValidatorAction {
        ...
        /** データレコード */
        private static final String DATA_RECORD_MSG = "PiTaPa利用実績情報レコード";
        ...
        public Result doData(DataRecord inputData, ExecutionContext ctx) {
            ...
            // 入力データ件数(PiTaPa利用実績情報レコード)をカウントアップする。
            B138A011Action.this.inputDataCount++;
        ...
        public void onFileEnd(ExecutionContext ctx) {
            ...
            if (!END_RECORD.equals(this.preRecordKbn)) {
                // 最終レコードがエンドレコードで無い場合
                writeLog("MB138A0104", END_RECORD_MSG);
                throw new TransactionAbnormalEnd(100, "AB138A0101");
            }

            // 入力データ件数(PiTaPa利用実績情報レコード)
            writeLog("MB138A0105", B138A011Action.this.inputDataCount);
        }
        ...
        private void checkHeader(DataRecord inputData) {
            ...
            // inputDataをFormに展開する
            A138A001HeaderForm headerForm = BeanUtil.createAndCopy(A138A001HeaderForm.class, inputData);
            ...
            if (!this.fileId.equals(inputData.getString("fileId"))) {
                // ファイルIDのその他精査はエラーとする
                writeLog("MB138A0102", HEADER_RECORD_MSG, FILE_ID_MSG);
                throw new TransactionAbnormalEnd(100, "AB138A0101");
            }
        }
        ...
        private void checkData(DataRecord inputData) {
            ...
            if (!HEADER_RECORD.equals(this.preRecordKbn) && !DATA_RECORD.equals(this.preRecordKbn)) {
                ...
                writeLog("MB138A0103", DATA_RECORD_MSG);
                throw new TransactionAbnormalEnd(100, "AB138A0101");
            }
        }
        ...
        private void checkTrailer(DataRecord inputData) {
            ...
            if (!HEADER_RECORD.equals(this.preRecordKbn) && !DATA_RECORD.equals(this.preRecordKbn)) {
                ...
                writeLog("MB138A0103", TRAILER_RECORD_MSG);
                throw new TransactionAbnormalEnd(100, "AB138A0101");
            }
            ...
            // レコード件数と一致すること
            if (B138A011Action.this.inputDataCount != Integer.parseInt(dataRecordCnt)) {
                // レコード件数と一致しない場合
                writeLog("MB138A0102", TRAILER_RECORD_MSG, DATA_RECORD_CNT_MSG);
                throw new TransactionAbnormalEnd(100, "AB138A0101");
            }
        }
